...
public abstract class DrawableTree extends Tree {
	...
	public double nodeDistance(DrawableNode n, Point pos) {
		return nodeDistance(n, pos.x, pos.y);
	}
	...
	public Tree extractSelectedTree() {
		...
		if (ancestor==null) {
			ErrorWindow.showErrorWindow(new NullPointerException("Could not find common ancestor of selected nodes"), SunFishFrame.getSunFishFrame().getLogger());
			return null;
		}
	...
	public double angleFromParent(DrawableNode n) {
		if (n.getParent()==null)
			...
			Point pPos = ((DrawableNode)n.getParent()).getPosition();
			if (pPos.x == n.getX() && pPos.y == n.getY()) {
...
//				System.out.println("Parent : " + n.getParent().getLabel());
	 ...
	 */
	public double angleFromPoint(Point to, Point from) {
		double val;
		if (from.y==to.y) {
			if (to.x<from.x)
				return 270.0;
		...
		double arg = (from.x-to.x )/(from.y-to.y);
		val =  -57.2957795*Math.atan( arg);
		
		if (Double.isNaN(val)) {
			System.out.println("Got NaN in AngleFromPoint, arg : " + arg + " from y: " + from.y + " to y : " + to.y);
		}
		if (from.y<to.y)
			return 180+val;
		else
	...
	protected double angleFromPos(DrawableNode n, Point pPos) {
	...
	protected static String subTreeLabel(String subtree) {
		...
		if (! hasParens) {
			...
			if (pos < 0) {
				SunFishFrame.getSunFishFrame().getLogger().warning("Found a broken subtree during newick subtree parse: " + subtree);
				return "";
			}
		...
		if (annoIndex > parenIndex) {
			...
			for(int i=0; i<annotations.length; i++) {
				...
				if (keyVal.length == 2) {
					...
					SunFishFrame.getSunFishFrame().getLogger().warning("Unrecognized node annotation : " + keyVal);
				}
			}
		}
	}
