package plugins.treePlugin.tree.drawing;
...
public class TreeDrawer implements TreeListener {
	...
	public void setTree(DrawableTree tree) {
		...
		if (tree instanceof EqualAngleTree) {
			defaultBranchPainter = new LineBranchPainter();
			textTransform = new AffineTransform();
			textTransform.rotate(Math.PI*1.5);
		}
	}
	...
	private void setDrawBounds() {
		//The drawing area must be square if we're concerned about having actual branch lengths that are proportional
		//to the branch lengths specified by the tree. This is only a concern for non-SquareTrees, so if the tree
		//is an EqualAngleTree adjust the padding appropriately
		if (tree instanceof EqualAngleTree) {
			topPadding = 0;
			bottomPadding = 0;		
			leftPadding = 0;
			...
			if ( drawWidth > drawHeight ) {
				...
				int dif = drawWidth-drawHeight;
				leftPadding += dif/2;
				rightPadding += dif/2;
			}
			else {
				int dif = drawHeight-drawWidth;
				topPadding +=  dif/2;
				bottomPadding += dif/2;
			}
	...
	public void initializeBoundaries(Rectangle bounds, Graphics2D g2d) {
		...
		if (tree instanceof EqualAngleTree) { 
			...
			DrawableNode maxNode = null;
			
			for(Node tip : tree.getAllTips()) {
				g2d.setFont( ((DrawableNode)tip).getFont());
				FontMetrics fm = g2d.getFontMetrics();	
				double nameLength = fm.stringWidth( ((DrawableNode)tip ).getCurrentLabel() );
				double angle = tree.angleFromParent((DrawableNode)tip);
				if (angle>360)
					angle -= 360;
				if (angle < 0)
					angle += 360;
				...
				if (angle>=45 && angle < 135) {
					if (nameLength > maxRight)
						maxRight = nameLength;
				}
				if (angle>=135 && angle < 225) {
					if (nameLength > maxDown)
						maxDown = nameLength;	
				}
				if (angle>=225 && angle < 315) {
					if (nameLength > maxLeft)
						maxLeft = nameLength;
				}
				if (nameLength > maxDist) {
					maxDist = nameLength;
					maxNode = (DrawableNode)tip;
				}
			}
			
//			leftPadding += maxDist;
//			topPadding += maxDist;
//			rightPadding += maxDist;
//			bottomPadding += maxDist;
			
			if (maxDist==maxLeft || maxDist == maxRight){
				leftPadding+= maxLeft;
				rightPadding+= maxRight;
				topPadding += (maxLeft+maxRight)/2.0;
				bottomPadding += (maxLeft+maxRight)/2.0;
				//System.out.println("max dist is left-right");
			}
			else {
				topPadding+= maxUp;
				bottomPadding+= maxDown;
				topPadding += (maxUp+maxDown)/2.0;
				bottomPadding += (maxDown+maxUp)/2.0;
				//System.out.println("max dist is up-down");
			}
			
			//System.out.println("Max name is:" + maxNode.getCurrentLabel() + " Max left: " + maxLeft + " max up: " + maxUp + " max right: " + maxRight + " max down: " + maxDown);
			//System.out.println("Max name width is: " + maxDist + " top: " + topPadding + " left: " + leftPadding + " bottom: " + bottomPadding + " right: " + rightPadding);
	...
	public void paint(Graphics2D g2d, Rectangle bounds) {
		...
		//System.out.println("Tree drawer thinks 0,0 is : " + translateTreeToPixelX(0));
		//System.out.println("Bounds x: " + bounds.x + " bounds width: " + bounds.width + " Branch scale: " + branchScale + " left padding: " + leftPadding + " label space: " + labelSpace);
//		AffineTransform trans = new AffineTransform();
//		trans.rotate(Math.PI*0.1, getWidth()/2, getHeight()/2);
...
//		g2d.drawRect(translateTreeToPixelX(0.0), translateTreeToPixelY(0.0), translateTreeToPixelX(1.0)-translateTreeToPixelX(0.0), translateTreeToPixelY(1.0)-translateTreeToPixelY(0.0));
	...
	protected java.awt.Point calculateLabelXY(DrawableNode n, int x, int y, FontMetrics fm) {
		...
		if (n.getLabelPosition()==TreeDisplay.RIGHT_POSITION) {
		...
		if (n.getLabelPosition()==TreeDisplay.UPPER_LEFT_POSITION) {
			workingPoint.y = y - 1;
			return workingPoint;
		}
		if (n.getLabelPosition()==TreeDisplay.LOWER_LEFT_POSITION) {	
	...
	protected void drawTipLabel(Graphics2D g2d, DrawableNode node, int x, int y) {
		...
		if (tree instanceof EqualAngleTree) {
			//Tip labels should be rotated to have same angle from parent 
			double angle = ((EqualAngleTree)tree).angleFromParent(node);
			double radAngle = angle*Math.PI/180.0-Math.PI/2.0; 
			FontMetrics fm = g2d.getFontMetrics();
			//System.out.println("Str width before rotation: " + fm.stringWidth("xxxx") + " str bounds width: " + fm.getStringBounds("xxxx", 0, 4, g2d).getWidth());
			if ( (angle>180 && angle < 360) || (angle<0 && angle>-180) ) {
				int width = (int)Math.round(fm.getStringBounds(theLabel, 0, theLabel.length(), g2d).getWidth());
				labX = (int)Math.round(x-(width*Math.cos(radAngle+Math.PI))-2);
				labY = y-(int)Math.round((width+2)*Math.sin(radAngle+Math.PI));
				double hCorrect = g2d.getFont().getSize2D()/2.0;
				labY += hCorrect*(0.5+Math.cos(radAngle+Math.PI)/2.0);
			
				textTransform.setToRotation(radAngle+Math.PI);
			}
			else {
				labX = x+(int)Math.round( 3*Math.cos(radAngle));
				if (angle>90) {
					double hCorrect = g2d.getFont().getSize2D()/2.0;
					labX -= hCorrect*(0.5+Math.sin(radAngle+Math.PI)/2.0);
				}
				if (angle<90) {
					labX += 2;
					labY -= 2;
				}
				textTransform.setToRotation(radAngle);
				
			}
			
			g2d.setFont(g2d.getFont().deriveFont(textTransform));
			//System.out.println("Str width AFTER rotation: " + fm.stringWidth("xxxxx") + " str bounds width: " + fm.getStringBounds("xxxx", 0, 4, g2d).getWidth());
			//theLabel = theLabel + String.valueOf((int)Math.round(angle));
		}//Equal angle tree tip drawing
